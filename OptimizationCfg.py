...
def logTermination(objTermination, fittest):
    ...
    logging.getLogger("Platypus").info(message)
...
def plottingSchwefel(x1, x2, lower, upper, run=False):
    ...
    plt.title('Schwefel Test Function')
    plt.show()
...
def plottingConvergence(x1, x2, lower, upper, solutions, max_display=3, run=False):
    if not run:
        return
    ...
    for algorithmName, solverSolution in solutions.items():
        for solutionName, solutionType in solverSolution.items():
            if solutionName == LogHeader.GENERATION.value:
                for cnt, iter_solution in enumerate(solutionType.items()):
                    if cnt < max_display:
                        iteration = iter_solution[0]
                        solution = iter_solution[1]
